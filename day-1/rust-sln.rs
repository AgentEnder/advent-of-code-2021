...
fn main() {
    let stdin = stdin();
    let lines = stdin.lock().lines().map(|x|x.unwrap().parse::<i32>().unwrap()).collect::<Vec<i32>>();
    
    let mut sliding_windows: Vec<i32> = vec![];
    let iter = lines.iter();
    for (i, num) in iter.enumerate() {
        if i > 0 {sliding_windows[i-1] = sliding_windows[i-1] + num}
        if i > 1 {sliding_windows[i-2] = sliding_windows[i-2] + num}
